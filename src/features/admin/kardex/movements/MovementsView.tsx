...
function tipoPill(tipo: string) {
    ...
    switch (t) {
        case 'INGRESO': return { label: 'Ingreso', dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50' };
        case 'SALIDA': return { label: 'Salida', dot: 'bg-rose-500', text: 'text-rose-600', bg: 'bg-rose-50' };
        case 'AJUSTE': return { label: 'Ajuste', dot: 'bg-blue-500', text: 'text-blue-600', bg: 'bg-blue-50' };
        case 'TRANSFERENCIA': return { label: 'Transferencia', dot: 'bg-violet-500', text: 'text-violet-600', bg: 'bg-violet-50' };
        default: return { label: t ? t.charAt(0) + t.slice(1).toLowerCase() : '—', dot: 'bg-slate-400', text: 'text-slate-500', bg: 'bg-slate-100' };
    }
}
...
export default function MovementsView() {
    ...
    return (
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Movimientos de Kardex</h1>
                        <p className="text-sm text-slate-400 mt-0.5">Control de entradas, salidas y ajustes de inventario</p>
                    </div>
                </div>
            </div>

            {/* Filtros */}
            <div className="mb-5 p-5 bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100">
                <div className="flex items-center gap-2.5 mb-4">
                    <div className="h-8 w-8 grid place-items-center rounded-xl bg-blue-50 text-blue-600">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="relative">
                        ...
                        {showSuggestions && products && products.length > 0 && (
                            <div className="absolute z-20 mt-1 w-full bg-white border border-slate-100 rounded-2xl shadow-xl max-h-60 overflow-auto">
                                {products.map((p: any) => (
                                    <button
                                        key={p.id}
                                        type="button"
                                        onClick={() => actions.selectProduct(p)}
                                        className="w-full text-left px-4 py-3 hover:bg-slate-50 text-sm border-b border-slate-50 last:border-b-0 transition-colors"
                                    >
                                        <div className="font-semibold text-slate-700">{p.descripcion}</div>
                                        <div className="text-xs text-slate-400">{p.codigo} • {p?.unidadMedida?.nombre}</div>
                                    </button>
                                ))}
                            </div>
                        )}
                ...
                <div className="grid grid-cols-2 sm:flex sm:justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            {loading && !kardex ? (
                <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100 flex justify-center items-center py-20">
                    <Icon icon="line-md:loading-twotone-loop" className="text-4xl" style={{ color: ACCENT }} />
                    <span className="ml-3 text-slate-400 font-medium">Cargando movimientos...</span>
                </div>
            ) : (
                <>
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100 overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                        <div className="h-8 w-8 grid place-items-center rounded-xl bg-violet-50 text-violet-600">
                ...
                {movimientosTable?.length > 0 ? (
                    <>
                            ...
                            <table className="w-full text-left border-collapse min-w-[1100px]">
                                ...
                                <tbody>
                                    {movimientosTable.map((row: any, index: number) => (
                                        <tr key={`${row.fecha}-${row.producto}-${index}`} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                            <td className="py-3 pl-5 pr-3 text-sm text-slate-500 whitespace-nowrap">{row.fecha}</td>
                                            <td className="py-3 px-3">
                                                <div className="flex items-center gap-2.5 min-w-0">
                                                    <div className="h-8 w-8 rounded-xl bg-violet-50 text-violet-600 grid place-items-center shrink-0">
                                                        <Icon icon="solar:box-linear" width={16} height={16} />
                                                    </div>
                                                    <span className="font-semibold text-slate-700 text-sm truncate max-w-[200px]" title={row.producto}>{row.producto}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[220px]" title={row.concepto}>{row.concepto}</td>
                                            <td className={`py-3 px-3 text-right text-sm font-bold whitespace-nowrap ${Number(row.cantidad) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                {Number(row.cantidad) > 0 ? '+' : ''}{row.cantidad}
                                            </td>
                                            <td className="py-3 px-3 text-right text-sm text-slate-500">{row.stockAnterior}</td>
                                            <td className="py-3 px-3 text-right text-sm font-bold text-slate-800">{row.stockActual}</td>
                                            <td className="py-3 px-3 text-right text-sm text-slate-500 whitespace-nowrap">{row.costoUnitario}</td>
                                            <td className="py-3 px-3 text-right text-sm text-slate-500 whitespace-nowrap">{row.precioUnitario}</td>
                                            <td className="py-3 px-3 text-right text-sm font-semibold text-slate-700 whitespace-nowrap">{row.gananciaUnidad}</td>
                                            <td className="py-3 px-3 pr-5 text-right">
                                                <button
                                                    type="button"
                                                    onClick={() => actions.openModal(row._original)}
                                                    className="h-8 w-8 grid place-items-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-violet-600 transition-colors"
                                                    title="Ver Detalle"
                        ...
                        <div className="md:hidden space-y-3 p-3">
                            {movimientosTable.map((row: any, index: number) => (
                                <article key={`${row.fecha}-${row.producto}-${index}`} className="rounded-2xl border border-slate-100 bg-white p-4 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="min-w-0">
                                            <p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">{row.fecha}</p>
                                            <h3 className="mt-1 line-clamp-2 text-sm font-extrabold text-slate-800">{row.producto}</h3>
                                            <p className="mt-1 text-xs font-semibold text-slate-400">{row.sede}</p>
                                        </div>
                                        <button
                                            type="button"
                                            onClick={() => actions.openModal(row._original)}
                                            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-violet-600"
                                            aria-label="Ver detalle"
                                        >
                                    ...
                                    <div className="mt-3 flex items-center justify-between gap-3">
                                        ...
                                        <p className={`text-lg font-extrabold ${Number(row.cantidad) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                            {Number(row.cantidad) > 0 ? '+' : ''}{row.cantidad}
                                        </p>
                                    </div>
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                                        <div className="rounded-xl bg-slate-50 p-3">
                                            <p className="font-bold uppercase tracking-wide text-slate-400">Stock anterior</p>
                                            <p className="mt-1 font-extrabold text-slate-800">{row.stockAnterior}</p>
                                        </div>
                                        <div className="rounded-xl bg-slate-50 p-3">
                                            <p className="font-bold uppercase tracking-wide text-slate-400">Stock actual</p>
                                            <p className="mt-1 font-extrabold text-slate-800">{row.stockActual}</p>
                                        </div>
                                        <div className="rounded-xl bg-slate-50 p-3">
                                            <p className="font-bold uppercase tracking-wide text-slate-400">Costo</p>
                                            <p className="mt-1 font-extrabold text-slate-800">{row.costoUnitario}</p>
                                        </div>
                                        <div className="rounded-xl bg-slate-50 p-3">
                                            <p className="font-bold uppercase tracking-wide text-slate-400">Precio</p>
                                            <p className="mt-1 font-extrabold text-slate-800">{row.precioUnitario}</p>
                                        </div>
                                    </div>
                                </article>
                            ))}
                    ...
                    <div className="py-16 text-center">
                        <Icon icon="solar:box-linear" className="text-5xl text-slate-200 mx-auto mb-3" />
                ...
                {selectedMovimiento && (
                    <div className="flex flex-col gap-6 p-6">
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 border-b border-slate-100 pb-2 mb-2">
                                    <Icon icon="solar:clipboard-list-linear" className="text-slate-400" />
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-slate-700 bg-slate-50 border border-slate-100 p-3 rounded-xl">
                                ...
                                {selectedMovimiento.lote && (
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-slate-800">{selectedMovimiento.lote}</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center gap-2 border-b border-slate-100 pb-2 mb-2">
                                    <Icon icon="solar:chart-square-linear" className="text-slate-400" />
                                    <h5 className="text-sm font-bold text-slate-800 uppercase tracking-wide">
                                        Valores y Stock
                                    </h5>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-slate-50 p-3 rounded-xl text-center">
                                        <label className="block text-xs font-medium text-slate-400 mb-1">Stock Anterior</label>
                                        <p className="text-lg font-semibold text-slate-700 leading-none">{parseFloat(Number(selectedMovimiento.stockAnterior ?? 0).toFixed(3))}</p>
                                    </div>
                                    <div className="bg-violet-50 p-3 rounded-xl border border-violet-100 text-center">
                                        <label className="block text-xs font-medium text-violet-600 mb-1">Stock Actual</label>
                                        <p className="text-lg font-bold text-violet-700 leading-none">{parseFloat(Number(selectedMovimiento.stockActual ?? 0).toFixed(3))}</p>
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-4 pt-2">
                                    <div>
                                        <label className="block text-xs font-medium text-slate-400 mb-1">Costo Unit.</label>
                                    ...
                                    {selectedMovimiento.valorTotal && (
                                        <div>
                                            <label className="block text-xs font-medium text-slate-400 mb-1">Valor Total</label>
                        ...
                        {(selectedMovimiento.usuario || selectedMovimiento.comprobante) && (
                            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 mt-auto flex flex-col gap-2 text-xs text-slate-500">
                                {selectedMovimiento.usuario && (
                                    <div className="flex items-center gap-2">
                                        <Icon icon="solar:user-circle-linear" width={16} />
                                        <span>Responsable: <span className="font-medium text-slate-700">{selectedMovimiento.usuario.nombre}</span></span>
                                    </div>
                                )}
                                {selectedMovimiento.comprobante && (
                                    <div className="flex items-center gap-2 border-t border-slate-200 pt-2">
                                        ...
                                        <span>Documento: <span className="font-medium text-slate-700">
