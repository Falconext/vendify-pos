        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Dashboard de Inventario</h1>
                ...
                <button
                    ...
                    className="h-11 px-4 rounded-2xl border-2 text-sm font-bold flex items-center gap-1.5 transition-colors shrink-0 bg-white hover:bg-slate-50"
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
                    {Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className="bg-white rounded-3xl p-5 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                            <div className="h-10 w-10 rounded-2xl bg-slate-100 animate-pulse mb-4" />
                            <div className="h-7 w-24 rounded-lg bg-slate-100 animate-pulse mb-2" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 mb-6">
                    <div className="bg-white rounded-3xl p-5 shadow-[0_2px_20px_rgba(15,23,42,0.05)] flex flex-col justify-between group hover:shadow-[0_4px_28px_rgba(15,23,42,0.08)] transition-shadow">
                        ...
                        <div>
                            <h2 className="text-[28px] leading-none font-extrabold text-slate-800 mb-2">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
                    <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                        <h3 className="text-slate-800 font-extrabold text-lg mb-1">Estado del stock</h3>
                        <TremorBarChart
                            className="mt-4 h-64"
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                        <h3 className="text-slate-800 font-extrabold text-lg mb-1">Distribución del inventario</h3>
                        <DonutChart
                            className="mt-4 h-64"
                            data={charts.pieData}
                            index="name"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <h3 className="font-extrabold text-slate-800 text-lg">Stock Crítico</h3>
                        </div>
                                    ...
                                    <div key={producto.id} className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-0.5">
                                                <p className="text-[13px] font-bold text-slate-800 truncate pr-2">{producto.descripcion}</p>
                                                <p className="text-[13px] font-bold text-slate-600 shrink-0">{helpers.formatCurrency(producto.valorTotal)}</p>
                                            </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <h3 className="font-extrabold text-slate-800 text-lg">Productos Obsoletos</h3>
                        </div>
                                ...
                                {dashboardData.topProductos.obsoletos.map((producto) => (
                                    <div key={producto.id} className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-baseline mb-0.5">
                                                <p className="text-[13px] font-bold text-slate-800 truncate pr-2">{producto.descripcion}</p>
                                                <p className="text-[13px] font-bold text-slate-600 shrink-0">{helpers.formatCurrency(producto.valorInmovilizado)}</p>
                                            </div>
                                            <div className="flex items-center gap-1">
                                                <span className="text-xs text-slate-400">{producto.codigo}</span>
                                                <span className="text-xs text-slate-500">· {producto.diasSinMovimiento} días sin movimiento</span>
                ...
                <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                    <div className="p-5 border-b border-slate-100 flex items-center gap-3">
                        ...
                        <h3 className="font-extrabold text-slate-800 text-lg">Movimientos Recientes</h3>
                    </div>
                    <div className="p-4">
                        {dashboardData.movimientosRecientes.length > 0 ? (
                            ...
                            <div className="text-center py-8">
                                <Icon icon="solar:inbox-linear" className="text-4xl text-slate-200 mx-auto mb-2" />
                ...
                <div className="mt-8">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="w-10 h-10 bg-violet-50 rounded-2xl flex items-center justify-center">
                            <Icon icon="solar:medical-kit-bold-duotone" className="text-violet-600 text-lg" />
                        </div>
                        <h2 className="text-lg font-extrabold text-slate-800">Control Farmacéutico</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-6">
                        {[
                            ...
                            <div key={kpi.label} className="bg-white rounded-3xl p-5 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center mb-3 ${kpi.color === 'rose' ? 'bg-rose-50' : kpi.color === 'amber' ? 'bg-amber-50' : 'bg-emerald-50'}`}>
                                    <Icon icon={kpi.icon} className={`text-xl ${kpi.color === 'rose' ? 'text-rose-600' : kpi.color === 'amber' ? 'text-amber-600' : 'text-emerald-600'}`} />
                                </div>
                            ...
                            {dashboardData.farmacia.top5PorVencer.length === 0 ? (
                                ...
                                <div className="divide-y divide-slate-50">
                                    {dashboardData.farmacia.top5PorVencer.map((l: any) => (
                                        <div key={l.id} className="flex items-center justify-between px-5 py-3 hover:bg-slate-50/60 transition-colors">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-semibold text-slate-800 truncate">{l.producto.descripcion}</p>
                                                <p className="text-xs text-slate-400 font-mono">{l.lote} · Stock: {l.stockActual}</p>
                                            </div>
                                            <span className="ml-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-50 text-amber-600 flex-shrink-0">
                                ...
                                <div className="divide-y divide-slate-50">
                                    {dashboardData.farmacia.top5Vencidos.map((l: any) => (
                                        <div key={l.id} className="flex items-center justify-between px-5 py-3 hover:bg-slate-50/60 transition-colors">
                                            ...
                                            <span className="ml-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-50 text-rose-600 flex-shrink-0">
