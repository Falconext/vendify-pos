...
export default function UsersView() {
    ...
    const getPermisosCell = (permisos: string[] = [], rol?: string) => {
        ...
        if (data.type === 'none') {
            return <span className="text-xs font-medium text-slate-400">{data.label}</span>;
        }
        if (data.type === 'full') {
            return (
                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-bold bg-emerald-50 text-emerald-600">
                    <Icon icon="solar:shield-check-bold" className="text-sm" /> {data.label}
                </span>
            );
        }
        return (
            <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-600" title={data.tooltip}>
                {data.label}
            </span>
        );
    };
        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Gestión de Usuarios</h1>
                    <p className="text-sm text-slate-400 mt-0.5">Administra los usuarios y sus permisos del sistema.</p>
                </div>
                    ...
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-3xl font-extrabold text-slate-800 mt-1">
                                {totalUsuarios}
                                {limiteUsuarios > 0 && <span className="text-lg font-bold text-slate-300"> / {limiteUsuarios}</span>}
                            </p>
                            {limiteUsuarios > 0 && (
                                <p className={`text-[11px] font-semibold mt-0.5 ${limiteAlcanzado ? 'text-amber-600' : 'text-slate-400'}`}>
                                    {limiteAlcanzado
                        ...
                        <div className="h-12 w-12 rounded-2xl bg-violet-50 text-violet-600 grid place-items-center shrink-0">
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2.5 p-4 border-b border-slate-100">
                    ...
                    {limiteAlcanzado && (
                        <span className="h-9 px-3.5 rounded-xl bg-amber-50 text-amber-600 text-sm font-semibold flex items-center gap-1.5">
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={8} className="py-16 text-center">
                                        <Icon icon="solar:users-group-rounded-linear" className="text-5xl text-slate-200 mx-auto mb-2" />
                                        <p className="text-slate-500 text-sm font-medium mb-1">No hay usuarios registrados</p>
                                ...
                                return (
                                    <tr key={usuario.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                        <td className="py-3 pl-5 pr-3">
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                ...
                                                <span className="font-semibold text-slate-700 text-sm truncate max-w-[180px]">{nombre}</span>
                                            </div>
                                        </td>
                                        <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[200px]">{usuario.email?.toLowerCase() || '—'}</td>
                                        <td className="py-3 px-3 font-mono text-sm text-slate-600">{usuario.dni || '—'}</td>
                                        <td className="py-3 px-3 text-sm text-slate-500">{usuario.celular || '—'}</td>
                                        <td className="py-3 px-3">
                                            <span className={`px-2 py-0.5 rounded-md text-[11px] font-bold ${esAdmin ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>
                                                {esAdmin ? 'Admin' : 'Usuario'}
                                            </span>
                                        </td>
                                        <td className="py-3 px-3">{getPermisosCell(usuario.permisos, usuario.rol)}</td>
                                        <td className="py-3 px-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${activo ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                        ...
                                        <td className="py-3 px-3 pr-5">
                                            <div className="flex items-center justify-end gap-1.5">
                                                <button
                                                    ...
                                                    title="Editar"
                                                    className="h-8 w-8 grid place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-violet-600 transition-colors"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="h-8 w-8 grid place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-amber-50 hover:text-amber-600 transition-colors"
                                                >
                ...
                {usuarios.length > 0 && (
                    <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-t border-slate-100">
                        <span className="text-sm text-slate-400">{fromRow}-{toRow} de {totalUsuarios.toLocaleString('es-PE')}</span>
                        <div className="flex items-center gap-1">
                            <button disabled={currentPage <= 1} onClick={() => handlePageChange(1)} className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30"><Icon icon="solar:double-alt-arrow-left-linear" /></button>
                            <button disabled={currentPage <= 1} onClick={() => handlePageChange(Math.max(1, currentPage - 1))} className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30"><Icon icon="solar:alt-arrow-left-linear" /></button>
                            {pageNumbers.map((n, i) => {
                            ...
                            <button disabled={currentPage >= totalPages} onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30"><Icon icon="solar:alt-arrow-right-linear" /></button>
                            <button disabled={currentPage >= totalPages} onClick={() => handlePageChange(totalPages)} className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 disabled:opacity-30"><Icon icon="solar:double-alt-arrow-right-linear" /></button>
                        </div>
