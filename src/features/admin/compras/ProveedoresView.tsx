        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight truncate">Proveedores</h1>
                    <p className="text-sm text-slate-400 mt-0.5">Gestión de proveedores para compras</p>
                </div>
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2.5 p-4 border-b border-slate-100">
                    ...
                    <span className="text-sm text-slate-400 font-medium px-1">{Number(totalClients ?? 0).toLocaleString('es-PE')} resultados</span>
                </div>
                                            ...
                                            <tr key={row.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                                <td className="py-3 pl-5 pr-3">
                                                    <div className="flex items-center gap-2.5 min-w-0">
                                                        ...
                                                        <span className="font-semibold text-slate-700 text-sm truncate max-w-[220px]">{nombre || '—'}</span>
                                                    </div>
                                                </td>
                                                <td className="py-3 px-3">
                                                    {row['Documento'] ? (
                                                        <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-600">{row['Documento']}</span>
                                                    ) : <span className="text-slate-300">—</span>}
                                                </td>
                                                <td className="py-3 px-3 font-mono text-sm text-slate-600">{row['Nro. Doc'] || '—'}</td>
                                                <td className="py-3 px-3 text-sm text-slate-500">{row['Celular'] || '—'}</td>
                                                <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[220px]">{row['Dirección'] || '—'}</td>
                                                <td className="py-3 px-3">
                                                    {row['Estado'] ? (
                                                    ...
                                                    <div className="flex justify-end">
                                                        <div className="relative inline-block" onClick={(e) => e.stopPropagation()}>
                                                            <button
                                                                ...
                                                                className="h-8 w-8 grid place-items-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-700 transition-colors"
                                                            >
