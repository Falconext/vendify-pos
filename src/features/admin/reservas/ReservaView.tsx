...
const estadoPill: Record<EstadoReserva, { label: string; dot: string; text: string; bg: string }> = {
  PENDIENTE: { label: 'Pendiente', dot: 'bg-amber-500', text: 'text-amber-600', bg: 'bg-amber-50' },
  CONFIRMADA: { label: 'Confirmada', dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50' },
  CANCELADA: { label: 'Cancelada', dot: 'bg-rose-500', text: 'text-rose-600', bg: 'bg-rose-50' },
}
    ...
    <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
        <div className="min-w-0">
          <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Reservas</h1>
          <p className="text-sm text-slate-400 mt-0.5">Reserva stock para pre-órdenes y planificación.</p>
        </div>
        <button
      ...
      {/* Filtros */}
      <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-4 mb-4">
              ...
              {vm.loading ? (
                ...
                <tr>
                  <td colSpan={7} className="py-16 text-center">
                    <Icon icon="solar:bookmark-linear" className="text-5xl text-slate-200 mx-auto mb-2" />
                ...
                return (
                  <tr key={reserva.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                    <td className="py-3 pl-5 px-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-semibold text-slate-700 text-sm truncate max-w-[200px]">{nombre}</span>
                          <span className="text-xs text-slate-400 truncate">{reserva.producto.sku || '-'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-3 font-bold text-slate-800 text-sm">{reserva.cantidad}</td>
                    <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[200px]">{reserva.motivo || '-'}</td>
                    ...
                    <td className="py-3 px-3 text-sm text-slate-500">{String(reserva.createdAt).slice(0, 10)}</td>
                    <td className="py-3 px-3 pr-5">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => vm.openEditModal(reserva)}
                          title="Editar"
                          className="h-8 w-8 grid place-items-center rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors"
                        >
                        ...
                        {reserva.estado !== 'CONFIRMADA' && (
                          <button
                            ...
                            title="Confirmar"
                            className="h-8 w-8 grid place-items-center rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors"
                          >
                        ...
                        {reserva.estado !== 'CANCELADA' && (
                          <button
                            ...
                            title="Cancelar"
                            className="h-8 w-8 grid place-items-center rounded-lg bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors"
                          >
                        ...
                        <button
                          ...
                          title="Eliminar"
                          className="h-8 w-8 grid place-items-center rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition-colors"
                        >
