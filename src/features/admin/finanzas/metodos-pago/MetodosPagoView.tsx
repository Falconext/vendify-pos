    ...
    return (
        <div className="space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[...Array(4)].map((_, i) => <div key={i} className="h-32 bg-slate-100 rounded-3xl animate-pulse" />)}
            </div>
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-4 space-y-3">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-800 text-sm">{metodo.metodo}</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                        {metodo.cantidad} pago(s) · {metodo.referencias} con operación/voucher
                    </p>
                    <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden mt-2 max-w-md">
                        <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: color }} />
                    </div>
                </div>
                <div className="text-right">
                    <p className="text-slate-400 uppercase tracking-wide text-[11px] font-semibold">Total</p>
                    <p className="text-lg font-extrabold text-slate-800">{formatSoles(metodo.total)}</p>
                </div>
            ...
            {expanded && (
                <div className="border-t border-slate-100 overflow-x-auto">
                    <table className="w-full text-xs min-w-[720px]">
                        <thead>
                            <tr className="text-[11px] font-bold uppercase tracking-wide text-slate-400 border-b border-slate-100 bg-slate-50/60">
                        ...
                        <tbody>
                            {metodo.items.map((item) => (
                                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                    <td className="px-5 py-3 text-slate-500">{formatDate(item.fecha)}</td>
                                    <td className="px-4 py-3 font-semibold text-slate-700">{item.documento}</td>
                                    ...
                                    <td className="px-5 py-3 text-right font-bold text-slate-800">{formatSoles(item.monto)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
    ...
    return (
        <div className="min-h-0 -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="min-w-0">
                    <div className="flex items-center gap-2.5 flex-wrap">
                        <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Métodos de pago</h1>
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                    <button
                        ...
                        className={`h-9 px-3.5 rounded-xl text-sm font-semibold border transition-colors flex items-center gap-1.5 ${vm.usarRango ? 'text-white border-transparent' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                    ...
                    {vm.usarRango ? (
                        <>
                            <input type="date" value={vm.fechaInicio} onChange={(e) => vm.setFechaInicio(e.target.value)} className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:outline-none focus:border-[var(--accent)]" />
                            <input type="date" value={vm.fechaFin} onChange={(e) => vm.setFechaFin(e.target.value)} className="h-9 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 focus:outline-none focus:border-[var(--accent)]" />
                        </>
                    ) : (
                        <>
                            <button onClick={() => vm.navegarMes(-1)} className="h-9 w-9 grid place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50">
                                <Icon icon="solar:alt-arrow-left-linear" />
                            </button>
                            <button onClick={() => vm.navegarMes(1)} disabled={vm.isCurrentOrFuture} className="h-9 w-9 grid place-items-center rounded-xl border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 disabled:opacity-30">
            ...
            {vm.isLoading ? <Skeleton /> : !data || data.metodos.length === 0 ? (
                <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] flex flex-col items-center justify-center py-20 text-center">
                    <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mb-4">
                        <Icon icon="solar:card-2-linear" className="text-3xl text-slate-300" />
                    </div>
                ...
                <div className="space-y-5">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <Kpi icon="solar:wallet-money-bold-duotone" tone="bg-emerald-50 text-emerald-600" label="Total cobrado" value={formatSoles(data.resumen.totalCobrado)} sub="pagos de ventas" />
                        <Kpi icon="solar:card-2-bold-duotone" tone="bg-violet-50 text-violet-600" label="Métodos usados" value={String(data.resumen.totalMetodos)} sub="canales de cobro" />
                        <Kpi icon="solar:checklist-minimalistic-bold-duotone" tone="bg-amber-50 text-amber-600" label="Con voucher/op." value={`${data.resumen.totalConReferencia}/${data.resumen.totalPagos}`} sub="respaldo registrado" />
                        <Kpi icon="solar:archive-check-bold-duotone" tone="bg-blue-50 text-blue-600" label="Respaldo legacy" value={String(data.resumen.totalRespaldo)} sub="comprobantes sin pago separado" />
                    </div>
                    ...
                    <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] px-5 py-4 flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-slate-600 mt-1">Cada método se calcula desde pagos reales; los comprobantes antiguos sin pago separado entran como respaldo para no perder montos.</p>
                        </div>
                        <div className="text-right shrink-0">
                            <p className="text-slate-400 uppercase tracking-wide text-[11px] font-semibold">Total conciliado</p>
                            <p className="font-extrabold text-slate-800 text-lg">{formatSoles(data.resumen.totalCobrado)}</p>
                        </div>
                    </div>
                </div>
            )}
