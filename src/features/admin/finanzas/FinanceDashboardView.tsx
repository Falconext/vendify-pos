...
export default function FinanceDashboardView() {
    ...
    const cardClass =
        'bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100 p-6 transition-all hover:shadow-[0_4px_28px_rgba(15,23,42,0.08)]';
        ...
        >
            ...
            <div className="mb-6 space-y-5">
                <div className="mb-5 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                        <h2 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Flujo de Caja</h2>
                        <p className="mt-1 text-sm text-slate-400">Ingresos, egresos y conciliación del periodo.</p>
                    </div>
                ...
                <div
                    className={`${isMobileFiltersOpen ? 'flex' : 'hidden'} flex-col gap-4 rounded-3xl border border-slate-100 bg-white p-4 shadow-[0_2px_20px_rgba(15,23,42,0.05)] md:flex md:flex-row md:flex-wrap md:items-end`}
                >
            ...
            {vm.isAdmin && vm.selectedUsuarioId && (
                <div className="mb-5 flex items-start gap-2 rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3 text-xs font-medium text-amber-700">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className={`${cardClass} min-h-[400px]`}>
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                <h2 className="text-lg font-extrabold text-slate-800">Flujo de Caja Real</h2>
                                <p className="text-sm text-slate-400 mt-0.5">Ingresos vs Egresos diarios</p>
                            </div>
                            <div className="flex gap-2">
                                <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 bg-slate-50 px-2.5 py-1 rounded-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className={cardClass}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-xs uppercase tracking-wide text-slate-400 font-bold mb-1.5">Total Por Cobrar</p>
                                    <h3 className="text-2xl font-extrabold text-slate-800">{vm.valueFormatter(vm.kpis?.porCobrar || 0)}</h3>
                                </div>
                                <div className={`${iconChip} bg-emerald-50 text-emerald-600`}>
                            ...
                            <p className="text-xs text-slate-400 mt-2">Pendiente de cobro a clientes</p>
                        </div>

                        <div className={cardClass}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-xs uppercase tracking-wide text-slate-400 font-bold mb-1.5">Total Por Pagar</p>
                                    <h3 className="text-2xl font-extrabold text-slate-800">{vm.valueFormatter(vm.kpis?.porPagar || 0)}</h3>
                                </div>
                                <div className={`${iconChip} bg-amber-50 text-amber-600`}>
                            ...
                            <p className="text-xs text-slate-400 mt-2">Pendiente de pago a proveedores</p>
                        </div>
                    </div>
                    ...
                    <div className={cardClass}>
                        <div className="flex items-start justify-between gap-4 mb-5">
                            <div>
                                <h2 className="text-lg font-extrabold text-slate-800">Ingresos por método de pago</h2>
                                <p className="text-sm text-slate-400 mt-0.5">
                                    Total cobrado según pagos reales del periodo.
                                </p>
                            </div>
                            <div className="text-right">
                                <p className="text-[11px] uppercase tracking-wide font-bold text-slate-400">Conciliado</p>
                                <p className="text-lg font-extrabold text-slate-800">{vm.valueFormatter(vm.conciliacion?.totalPorMetodo || 0)}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {(vm.metodosPago || []).length === 0 ? (
                                <div className="rounded-2xl bg-slate-50 p-6 text-center text-sm text-slate-400">
                                    <Icon icon="solar:wallet-linear" className="text-3xl text-slate-200 mx-auto mb-1.5" />
                                ...
                                vm.metodosPago.map((item: any) => {
                                    ...
                                    return (
                                        <div key={item.metodo} className="rounded-2xl border border-slate-100 p-4">
                                            <div className="flex items-start justify-between gap-3">
                                                <div>
                                                    <p className="text-sm font-bold text-slate-800">{item.metodo}</p>
                                                    <p className="text-xs text-slate-400 mt-0.5">{item.explicacion}</p>
                                                </div>
                                                <div className="text-right shrink-0">
                                                    <p className="text-sm font-extrabold text-slate-800">{vm.valueFormatter(item.total)}</p>
                                                    <p className="text-[11px] text-slate-400">{item.cantidad} pago(s)</p>
                                                </div>
                                            </div>
                        ...
                        {(vm.conciliacion?.comprobantesRespaldo ?? 0) > 0 && (
                            <div className="mt-4 rounded-2xl bg-amber-50 border border-amber-100 p-3 text-xs font-semibold text-amber-700">
                ...
                <div className="space-y-6">
                    ...
                    <div className={cardClass}>
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="font-extrabold text-slate-800">Resumen Rápido</h3>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 hover:bg-violet-50 transition-colors group">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-xl bg-white text-violet-600 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                        <Icon icon="solar:card-send-bold-duotone" />
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-slate-800">Egresos</p>
                                        <p className="text-xs text-slate-400">Gastos operativos</p>
                                    </div>
                                </div>
                                <span className="text-sm font-extrabold text-slate-800">{vm.valueFormatter(vm.kpis?.egresosPeriodo || 0)}</span>
                            </div>

                            <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 hover:bg-emerald-50 transition-colors group">
                                <div className="flex items-center gap-3">
                                ...
                                <span className={`text-sm font-extrabold ${(vm.kpis?.balancePeriodo || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
