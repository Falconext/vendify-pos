...
const ArqueoCaja = () => {
    ...
    const kpis = resumen
        ? [
            { label: 'Total Efectivo', value: resumen.detalleEfectivo, icon: 'solar:banknote-2-linear', chip: 'bg-emerald-50 text-emerald-600', money: true },
            { label: 'Total Digital', value: resumen.totalDigital, icon: 'solar:smartphone-linear', chip: 'bg-blue-50 text-blue-600', money: true },
            { label: 'Total Tarjetas', value: resumen.totalTarjetas, icon: 'solar:card-linear', chip: 'bg-violet-50 text-violet-600', money: true },
            { label: 'Comprobantes', value: resumen.totalComprobantes, icon: 'solar:document-text-linear', chip: 'bg-amber-50 text-amber-600', money: false },
            { label: 'Total Ingresos', value: resumen.totalIngresos, icon: 'solar:wallet-money-linear', chip: 'bg-indigo-50 text-indigo-600', money: true },
        ]
        : [];
        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Arqueo de Caja</h1>
                    <p className="text-sm text-slate-400 mt-0.5">Resumen de ingresos y movimientos por medio de pago.</p>
                </div>
                <Button
            ...
            {resumen && (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-5">
                    {kpis.map((k) => (
                        <div key={k.label} className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-5">
            ...
            {resumen && (
                <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-6 mb-5">
                    <div className="flex items-center gap-2.5 mb-4">
                        <div className="h-8 w-8 rounded-xl grid place-items-center bg-violet-50 text-violet-600">
                            <Icon icon="solar:wallet-linear" className="text-lg" />
                        </div>
                        <h3 className="text-base font-extrabold text-slate-800">Detalle por Medio de Pago</h3>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                        {[['Efectivo', resumen.detalleEfectivo], ['Yape', resumen.detalleYape], ['Plin', resumen.detallePlin], ['Transferencia', resumen.detalleTransferencia], ['Tarjeta', resumen.detalleTarjeta]].map(([label, val]) => (
                            <div key={label as string} className="text-center p-4 rounded-2xl bg-slate-50 border border-slate-100">
                                <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label as string}</p>
                                <p className="text-lg font-extrabold text-slate-800 mt-1">S/ {(val as number).toFixed(2)}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* Movimientos */}
            <div className="w-full bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-6">
                <div className="flex items-center gap-2.5 mb-4">
                    <div className="h-8 w-8 rounded-xl grid place-items-center bg-blue-50 text-blue-600">
                        <Icon icon="solar:sort-by-time-linear" className="text-lg" />
                    </div>
                    <h3 className="text-base font-extrabold text-slate-800">Movimientos de Caja</h3>
                </div>
                {vm.movimientos?.length > 0 ? (
                    ...
                    <div className="py-16 text-center">
                        ...
                        <p className="text-slate-500 font-medium">No hay movimientos de caja</p>
