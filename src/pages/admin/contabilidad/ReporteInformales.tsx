...
const ReportesComprobantesInformales = () => {
    ...
    const resumenItems: Array<[string, number, string]> = r
        ? [
            ['Tickets', r.totalTickets, 'bg-violet-50 text-violet-600'],
            ['Notas de Venta', r.totalNotasVenta, 'bg-blue-50 text-blue-600'],
            ['Recibos por Honorarios', r.totalRecibosHonorarios, 'bg-emerald-50 text-emerald-600'],
            ['Comprobantes de Pago', r.totalComprobantesPago, 'bg-amber-50 text-amber-600'],
            ['Notas de Pedido', r.totalNotasPedido, 'bg-rose-50 text-rose-600'],
            ['Órdenes de Trabajo', r.totalOrdenesTrabajo, 'bg-indigo-50 text-indigo-600'],
        ]
        : [];
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight truncate">Reporte Interno</h1>
                        <p className="text-sm text-slate-400 mt-0.5">Ventas internas y documentos informales</p>
                    </div>
                </div>
                <button
                    type="button"
                ...
                {vm.reports?.length > 0 ? (
                    <>
                        ...
                        <div className="flex flex-wrap items-center gap-2.5 p-4 border-b border-slate-100">
                            <span className="inline-flex items-center gap-2 h-9 px-3.5 rounded-xl bg-violet-50 text-sm font-bold" style={{ color: ACCENT }}>
                                <Icon icon="solar:document-text-linear" />
                                {totalRegistros.toLocaleString('es-PE')} comprobante{totalRegistros === 1 ? '' : 's'}
                            </span>
                            <span className="text-sm text-slate-400 font-medium px-1 hidden sm:inline">Detalle de documentos internos</span>
                        </div>
                        ...
                        {r !== null && (
                            <div className="flex justify-end p-4 border-t border-slate-100">
                                <div className="w-full sm:min-w-[320px] sm:w-auto bg-[#F7F8FB] p-5 rounded-2xl border border-slate-100">
                                    <p className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-3">Resumen del período</p>
                                    {resumenItems.map(([label, val, chip]) => (
                                        <div key={label} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                                            ...
                                            <span className="text-sm font-bold text-slate-800">S/ {val.toFixed(2)}</span>
                                        </div>
                                    ))}
