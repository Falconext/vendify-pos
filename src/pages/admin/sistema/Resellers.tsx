...
export default function AdminResellers() {
    ...
    return (
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta space-y-6" style={{ ['--accent' as any]: ACCENT }}>
            {/* Breadcrumb */}
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Distribuidores</h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
                {[
                    ...
                    const tones: Record<string, string> = {
                        slate: 'bg-slate-50 text-slate-500', violet: 'bg-[#7551FF]/10 text-[#7551FF]',
                        emerald: 'bg-emerald-50 text-emerald-600', rose: 'bg-rose-50 text-rose-600',
                    };
                    return (
                        <div key={c.label} className={`rounded-2xl bg-white p-4 shadow-sm border ${c.tone === 'rose' ? 'border-rose-200' : 'border-slate-100'}`}>
                            ...
                            <p className={`mt-0.5 text-xl font-extrabold tabular-nums ${c.tone === 'rose' && kpis.riesgo > 0 ? 'text-rose-600' : c.tone === 'violet' ? 'text-[#7551FF]' : 'text-slate-800'}`}>{c.value}</p>
            ...
            {kpis.riesgo > 0 && (
                <button onClick={() => { setSoloRiesgo(true); setEstadoFilter('todos'); }}
                    className="w-full flex items-center gap-2.5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 hover:bg-rose-100 transition-colors text-left">
                ...
                <div className="flex flex-wrap items-center gap-2.5 p-4 border-b border-slate-100">
                    <div className="relative flex-1 min-w-[200px] max-w-xs">
                        ...
                        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nombre, código o email…"
                            className="w-full h-10 pl-10 pr-3 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:border-[#7551FF]" />
                    </div>
                    {/* Filtro estado */}
                    <div className="flex items-center gap-1 rounded-xl bg-slate-100 p-1">
                        {([['todos', 'Todos'], ['activos', 'Activos'], ['inactivos', 'Inactivos']] as const).map(([k, l]) => (
                            <button key={k} onClick={() => setEstadoFilter(k)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${estadoFilter === k ? 'bg-white text-[#7551FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{l}</button>
                        ))}
                    </div>
                    {/* Solo en riesgo */}
                    <button onClick={() => setSoloRiesgo((v) => !v)}
                        className={`h-9 px-3 rounded-xl text-xs font-bold flex items-center gap-1.5 border transition-colors ${soloRiesgo ? 'border-rose-300 bg-rose-50 text-rose-600' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                    ...
                    <div className="ml-auto flex items-center gap-1.5 text-xs text-slate-400">
                        ...
                        <select value={sortBy} onChange={(e) => setSortBy(e.target.value as any)}
                            className="h-9 rounded-xl border border-slate-200 bg-white pl-2.5 pr-7 text-xs font-semibold text-slate-600 focus:outline-none focus:border-[#7551FF]">
                            ...
                            {filteredResellers.length === 0 ? (
                                <tr>
                                    <td colSpan={10} className="py-16 text-center">
                                        <Icon icon="solar:users-group-two-rounded-linear" className="text-5xl text-slate-200 mx-auto mb-2" />
                                        <p className="text-slate-500 text-sm font-medium">{vm.resellers.length === 0 ? 'No hay distribuidores registrados' : 'Ningún distribuidor coincide con el filtro'}</p>
                                ...
                                filteredResellers.map((reseller: any) => {
                                    ...
                                    return (
                                        <tr key={reseller.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                            <td className="py-3 pl-5 pr-3">
                                                <div className="flex items-center gap-2.5 min-w-0">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-semibold text-slate-700 text-sm truncate max-w-[200px]">{reseller.nombre}</p>
                                                        <span className="font-mono text-[11px] text-[#7551FF]">{reseller.codigo}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[140px]">{reseller.representante || '—'}</td>
                                            ...
                                            <td className={`py-3 px-3 text-right text-sm font-semibold tabular-nums ${riesgo ? 'text-rose-600' : 'text-slate-700'}`}>
                                            ...
                                            <td className="py-3 px-3 text-right text-sm text-slate-600 tabular-nums">S/ {Number(metric?.mrrBruto || 0).toFixed(2)}</td>
                                            <td className="py-3 px-3 text-right">
                                                <div className="text-sm text-slate-700 tabular-nums">S/ {Number(metric?.margenMensual || 0).toFixed(2)}</div>
                                                <div className="text-[11px] text-emerald-600 font-bold tabular-nums">{Number(metric?.margenPct || 0).toFixed(1)}%</div>
                                            </td>
                                            <td className="py-3 px-3 text-right">
                                                <div className="text-sm font-semibold text-slate-700 tabular-nums">{Number(metric?.churn30dPct || 0).toFixed(1)}%</div>
                                                <div className="text-[11px] text-slate-400">{metric?.clientesPerdidos30d || 0} perdidos</div>
                                            </td>
                                            <td className="py-3 px-3 text-center">
                                                <span className="inline-flex items-center justify-center bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full text-xs font-bold tabular-nums">{reseller._count?.empresas || 0}</span>
                                            </td>
                                            <td className="py-3 px-3 text-center">
                                                <div className="flex flex-col items-center gap-1">
                                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${reseller.activo ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                                        <span className={`w-1.5 h-1.5 rounded-full ${reseller.activo ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                                                        {reseller.activo ? 'Activo' : 'Inactivo'}
                                                    </span>
                                                    {riesgo && <span className="inline-flex items-center gap-1 text-[10px] font-bold text-rose-600"><Icon icon="solar:danger-triangle-bold" width="11" /> Riesgo saldo</span>}
                                                </div>
                                            </td>
                                            <td className="py-3 px-3 pr-5">
                                                <div className="flex items-center justify-end gap-1">
                                                    ...
                                                    <button onClick={() => vm.openEditModal(reseller)} title="Editar" className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors"><Icon icon="solar:pen-bold-duotone" width="18" /></button>
                                                    <button onClick={() => vm.handleToggleEstado(reseller)} title={reseller.activo ? 'Desactivar' : 'Activar'} className={`h-8 w-8 grid place-items-center rounded-lg transition-colors ${reseller.activo ? 'text-rose-500 hover:bg-rose-50' : 'text-emerald-600 hover:bg-emerald-50'}`}><Icon icon={reseller.activo ? 'solar:forbidden-circle-bold-duotone' : 'solar:check-circle-bold-duotone'} width="18" /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                <div className="p-5 border-b border-slate-100 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-extrabold text-slate-800">Simulador · ¿Cuánto gana la plataforma?</h3>
                            <p className="text-sm text-slate-400">Ingreso mayorista según los tramos de descuento por cantidad de clientes.</p>
                        </div>
                    </div>
                    <div className="flex items-end gap-3">
                        <div>
                            <label className="text-[11px] font-bold text-slate-500 uppercase tracking-wide">Tu costo por cliente/mes</label>
                            <div className="relative mt-1">
                                ...
                                <input type="number" min="0" step="0.10" value={costoPlataforma}
                                    ...
                                    className="w-40 rounded-xl border border-slate-200 bg-slate-50 pl-9 pr-3 py-2 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#7551FF]/20 focus:border-[#7551FF]" />
                            </div>
                        ...
                        <div className="flex gap-1 rounded-2xl bg-slate-100 p-1 mx-5 mt-4">
                            {([['recargar', 'Recargar', 'solar:add-circle-bold-duotone'], ['ajustar', 'Corregir saldo', 'solar:pen-2-bold-duotone']] as const).map(([k, l, ic]) => (
                                <button key={k} type="button" onClick={() => vm.setSaldoMode(k)}
                                    className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-bold transition-colors ${vm.saldoMode === k ? 'bg-white text-[#7551FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
