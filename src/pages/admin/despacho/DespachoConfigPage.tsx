        ...
        <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100 p-6">
            <div className="flex items-start justify-between gap-4 mb-5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                            ...
                            <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-600">{estadoLabel}</span>
                        </p>
                    </div>
                </div>
                {/* Toggle */}
                <button
                    ...
                    className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${habilitado ? 'bg-emerald-500' : 'bg-slate-300'}`}
                >
            ...
            {habilitado && (
                <div className="space-y-4">
                    <div>
                        <label className="block text-xs font-bold uppercase tracking-wide text-slate-400 mb-1.5">Mensaje</label>
                        <textarea
                            value={mensaje}
                            onChange={e => onMensaje(e.target.value)}
                            placeholder={placeholder}
                            rows={3}
                            className="w-full rounded-2xl border-2 border-slate-200 bg-white px-3.5 py-2.5 text-sm text-slate-700 placeholder:text-slate-400 outline-none focus:border-[var(--accent)] transition-colors resize-none"
                        />
                        <p className="mt-2 text-[11px] text-slate-400 flex flex-wrap items-center gap-1">
                            <span className="font-semibold text-slate-500">Variables:</span>
                            <code className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-md font-mono">{'{{nombre}}'}</code>
                            <code className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-md font-mono">{'{{pedido}}'}</code>
                            <code className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-md font-mono">{'{{repartidor}}'}</code>
                            <code className="bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-md font-mono">{'{{empresa}}'}</code>
                        </p>
                    </div>

                    <div className="rounded-2xl bg-emerald-50 border border-emerald-100 p-3.5">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-600 mb-1.5 flex items-center gap-1">
                            <Icon icon="mdi:whatsapp" className="text-sm" />
                            Vista previa
                        </p>
                        <p className="text-xs text-slate-700 leading-relaxed">{preview}</p>
                    </div>
                </div>
            )}
        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="h-11 w-11 flex items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 shrink-0">
                        <Icon icon="mdi:whatsapp" className="text-xl" />
                    </div>
                    <div className="min-w-0">
                        <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight truncate">Notificaciones de Despacho</h1>
                        <p className="text-sm text-slate-400 mt-0.5">Mensajes automáticos vía WhatsApp al cliente</p>
                    </div>
                </div>
                <button
                    type="button"
            ...
            {/* Info banner */}
            <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3.5 flex items-start gap-3">
                <div className="h-8 w-8 flex items-center justify-center rounded-xl bg-blue-100 text-blue-600 shrink-0">
            ...
            {loading ? (
                <div className="space-y-4">
                    {Array.from({ length: 2 }).map((_, i) => (
                        <div key={i} className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] border border-slate-100 p-6">
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <div className="flex-1 space-y-2">
                                    ...
                                    <div className="h-3 w-64 rounded-lg bg-slate-100 animate-pulse" />
                                </div>
                            </div>
