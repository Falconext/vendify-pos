...
const NotificacionesPage = () => {
  ...
  const stats = [
    { label: 'Total', value: totalNotificaciones, icon: 'solar:bell-bold', chip: 'bg-violet-50 text-violet-600', num: 'text-slate-800' },
    { label: 'Sin Leer', value: sinLeer, icon: 'solar:letter-unread-bold', chip: 'bg-rose-50 text-rose-600', num: 'text-rose-600' },
    { label: 'Leídas', value: leidas, icon: 'solar:letter-opened-bold', chip: 'bg-emerald-50 text-emerald-600', num: 'text-emerald-600' },
  ];
  ...
  return (
    <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
        <div>
          <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Notificaciones</h1>
          <p className="text-sm text-slate-400 mt-0.5">Centro de alertas y mensajes del sistema</p>
        </div>
        {sinLeer > 0 && (
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-5">
      ...
      {vm.loading && vm.notificaciones.length === 0 ? (
        ...
        <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] p-12 text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-slate-50 mb-4">
            <Icon icon="solar:bell-off-linear" className="h-10 w-10 text-slate-300" />
          </div>
          <h3 className="text-xl font-extrabold text-slate-800 mb-2">No tienes notificaciones</h3>
          <p className="text-slate-400 max-w-md mx-auto">Cuando recibas notificaciones importantes, aparecerán aquí.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {vm.notificaciones?.map((notificacion) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] hover:shadow-[0_8px_30px_rgba(15,23,42,0.10)] transition-all cursor-pointer overflow-hidden ${!notificacion.leida ? 'ring-2 ring-[var(--accent)]/40' : ''}`}
              >
                <div className="p-4 pb-3 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-slate-800 text-sm line-clamp-2 leading-tight">{notificacion.titulo}</h3>
                      {!notificacion.leida && (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-violet-50 text-violet-600 mt-1">
                ...
                <div className="p-4">
                  <p className="text-sm text-slate-500 mb-3 line-clamp-3 leading-relaxed">{notificacion.mensaje}</p>
