        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight truncate">Gestión de Sedes</h1>
                    <p className="text-sm text-slate-400 mt-0.5">Administra las sucursales y almacenes del sistema</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2.5 p-4 border-b border-slate-100">
                    <div className="h-9 w-9 grid place-items-center rounded-xl bg-violet-50 text-violet-600 shrink-0">
                        <Icon icon="solar:city-bold-duotone" width={18} />
                    </div>
                    <span className="text-sm font-bold text-slate-700">Sucursales y almacenes</span>
                    <span className="text-sm text-slate-400 font-medium px-1">{vm.sedes.length.toLocaleString('es-PE')} registradas</span>
                </div>
                            ...
                            {vm.sedes.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="py-16 text-center">
                                        <Icon icon="solar:city-linear" className="text-5xl text-slate-200 mx-auto mb-2" />
                                        <p className="text-slate-400 text-sm">No hay sedes registradas.</p>
                                    </td>
                                </tr>
                            ) : vm.sedes.map((sede) => {
                                ...
                                return (
                                    <tr key={sede.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors group">
                                        <td className="py-3 pl-5 pr-3">
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                <div className={`h-8 w-8 rounded-xl grid place-items-center shrink-0 ${esAlmacen ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                                                    <Icon icon={esAlmacen ? 'solar:box-bold-duotone' : 'solar:shop-bold-duotone'} width={16} />
                                                </div>
                                                <span className="font-semibold text-slate-700 text-sm truncate max-w-[220px]">{sede.nombre}</span>
                                            </div>
                                        </td>
                                        <td className="py-3 px-3 text-sm text-slate-500 truncate max-w-[240px]">{sede.direccion || '—'}</td>
                                        <td className="py-3 px-3">
                                            <span className="font-mono text-sm text-slate-600">{sede.codigo || '—'}</span>
                                        </td>
                                        <td className="py-3 px-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${esAlmacen ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                                                ...
                                                {sede.esPrincipal && !esAlmacen && (
                                                    <span className="ml-0.5 px-1.5 py-0.5 rounded-md text-[9px] font-bold bg-indigo-50 text-indigo-600 uppercase">Principal</span>
                                                )}
                                            </span>
                                        </td>
                                        <td className="py-3 px-3">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${isActivo ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                        ...
                                        <td className="py-3 px-3 pr-5">
                                            <div className="flex items-center justify-end gap-1.5">
                                                <button
                                                    ...
                                                    title="Editar"
                                                    className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-colors"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-amber-50 hover:text-amber-600 transition-colors"
                                                >
                                                ...
                                                {canDelete && (
                                                    <button
                                                        ...
                                                        title="Eliminar"
                                                        className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-600 transition-colors"
                                                    >
