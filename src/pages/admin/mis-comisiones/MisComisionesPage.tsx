        ...
        <div className="min-h-screen -m-5 p-5 bg-[#F7F8FB] font-jakarta" style={{ ['--accent' as any]: ACCENT }}>
            ...
            <motion.div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-5" variants={fadeUp} initial="initial" animate="animate">
                <div className="min-w-0">
                    <h1 className="text-[22px] font-extrabold text-slate-800 tracking-tight">Mis Comisiones</h1>
                    <p className="text-sm text-slate-400 mt-0.5">Tus comisiones generadas en {MESES[mes - 1]} {anio}.</p>
                </div>

                <div className="flex items-center gap-1 bg-white rounded-2xl px-2 py-2 shadow-[0_2px_20px_rgba(15,23,42,0.05)]">
                    <button
                        onClick={() => navegarMes(-1)}
                        className="h-8 w-8 grid place-items-center rounded-xl text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors"
                    >
                        <Icon icon="solar:alt-arrow-left-linear" className="text-base" />
                    </button>
                    <span className="text-sm font-bold text-slate-800 px-3 min-w-[110px] text-center">
                        {MESES_CORTO[mes - 1]} {anio}
                    </span>
                    <button
                        onClick={() => navegarMes(1)}
                        disabled={esMesActual}
                        className="h-8 w-8 grid place-items-center rounded-xl text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
            ...
            {/* ── Panel de filtros ── */}
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] px-4 py-3 mb-5">
                    ...
                    {PRESETS.map((p) => {
                        ...
                        return (
                            <button
                                ...
                                className={`inline-flex items-center gap-1.5 h-9 px-3.5 rounded-xl text-xs font-bold transition-all ${active ? 'text-white shadow-lg shadow-violet-500/30' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                    ...
                    <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto sm:ml-auto">
                        ...
                        {hayFiltros && (
                            <button
                                ...
                                className="inline-flex items-center gap-1 h-9 px-2.5 rounded-xl text-xs font-bold hover:bg-slate-50 transition-colors"
                                style={{ color: ACCENT }}
                            >
                    ...
                    {hayFiltros && (
                        ...
                        >
                            ...
                            <span className="text-xs text-slate-400">
                                Mostrando <strong className="text-slate-700">{comisionesFiltradas.length}</strong> registro{comisionesFiltradas.length !== 1 ? 's' : ''} del período <strong style={{ color: ACCENT }}>{desde === hasta ? desde : `${desde} → ${hasta}`}</strong>
                            </span>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            >
                {kpis.map((kpi, i) => (
                    <motion.div
                        key={i}
                        className="bg-white rounded-3xl p-5 shadow-[0_2px_20px_rgba(15,23,42,0.05)] flex items-center justify-between gap-4"
                    ...
                    >
                        <div className="min-w-0">
                            <h3 className="text-slate-400 uppercase tracking-wide text-xs font-bold">{kpi.label}</h3>
                            <h2 className="text-[26px] leading-none font-extrabold text-slate-800 mt-2">{kpi.value}</h2>
                        </div>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                    ...
                                    <td className="py-3 px-3 text-sm text-slate-600 truncate max-w-[240px]">
                                        {c.descripcion ?? '—'}
                                    </td>
                                    <td className="py-3 px-3">
                                        <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-600">{c.cantidad}</span>
                                    </td>
                                    <td className="py-3 px-3 font-bold text-slate-800 text-sm">{fmt(c.montoComision)}</td>
