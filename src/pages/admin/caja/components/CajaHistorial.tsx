...
const tipoConfig = (tipo?: string) => {
    switch (tipo) {
        case 'APERTURA':
            return { label: 'Apertura', dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50', icon: 'solar:play-circle-bold' };
        case 'CIERRE':
            return { label: 'Cierre', dot: 'bg-rose-500', text: 'text-rose-600', bg: 'bg-rose-50', icon: 'solar:stop-circle-bold' };
        case 'INGRESO':
            return { label: 'Ingreso', dot: 'bg-blue-500', text: 'text-blue-600', bg: 'bg-blue-50', icon: 'solar:arrow-right-up-bold' };
        default:
            return { label: 'Egreso', dot: 'bg-amber-500', text: 'text-amber-600', bg: 'bg-amber-50', icon: 'solar:arrow-left-down-bold' };
    }
};
...
const CajaHistorial: React.FC = () => {
    ...
    return (
        <div className="animate-in fade-in" style={{ ['--accent' as any]: ACCENT }}>
            <div className="bg-white rounded-3xl shadow-[0_2px_20px_rgba(15,23,42,0.05)] overflow-hidden">
                ...
                <div className="p-5 border-b border-slate-100">
                    <div className="flex items-center gap-2.5 mb-4">
                        ...
                        <h3 className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Filtros de búsqueda</h3>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2.5 mt-4 justify-end">
                        ...
                        <button
                            ...
                            className="h-10 px-4 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 flex items-center gap-1.5 hover:bg-slate-50 transition-colors"
                        >
                            ...
                            {loading ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="py-16 text-center">
                                        ...
                                        <p className="text-slate-400 text-sm">No hay movimientos para mostrar.</p>
                                    </td>
                                </tr>
                                ...
                                return (
                                    <tr key={mov.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                                        ...
                                        <td className="py-3 px-3">
                                            <div className="flex items-center gap-2.5 min-w-0">
                                                ...
                                                <span className="font-semibold text-slate-700 text-sm truncate max-w-[160px]">{usuario}</span>
                                            </div>
                                        </td>
                                        <td className="py-3 px-3 font-bold text-slate-800 text-sm whitespace-nowrap">
                                            {formatCurrency(mov.monto ?? mov.montoInicial ?? mov.montoFinal ?? 0)}
                                        </td>
                                        <td className="py-3 px-3">
                                            {mov.categoriaGasto ? (
                                                <span className="px-2 py-0.5 rounded-md text-[11px] font-bold bg-indigo-50 text-indigo-600 whitespace-nowrap">
                                                    {mov.categoriaGasto}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="py-3 px-3">
                                            <span className="text-sm text-slate-500 max-w-[220px] truncate block">
                                                {detalle || '—'}
                                            </span>
                                        </td>
                                            ...
                                            {mov.tipoMovimiento === 'EGRESO' ? (
                                                <div className="flex items-center justify-end gap-1.5">
                                                    <button
                                                        ...
                                                        className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-violet-50 hover:text-violet-600 transition-colors"
                                                        title="Editar gasto"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="h-8 w-8 grid place-items-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-500 transition-colors"
