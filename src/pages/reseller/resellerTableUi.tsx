...
export const THEAD_TR = 'text-[11px] font-bold uppercase tracking-wide text-slate-400 border-b border-slate-100';
export const BODY_TR = 'border-b border-slate-50 hover:bg-slate-50/60 transition-colors group';
...
export function estadoPill(estado?: string): Pill {
    ...
    if (['ACTIVO', 'ACEPTADO', 'APLICADO', 'APROBADO', 'COMPLETADO', 'INGRESO'].includes(e))
        return { label: cap(e), dot: 'bg-emerald-500', text: 'text-emerald-600', bg: 'bg-emerald-50' };
    if (['PENDIENTE', 'ENVIANDO', 'EN_PROCESO', 'PROCESANDO'].includes(e))
        return { label: 'Pendiente', dot: 'bg-amber-500', text: 'text-amber-600', bg: 'bg-amber-50' };
    if (['DEMO', 'PLACEHOLDER', 'AJUSTE'].includes(e))
        return { label: 'Demo', dot: 'bg-amber-500', text: 'text-amber-600', bg: 'bg-amber-50' };
    if (['RECHAZADO', 'FALLIDO_ENVIO', 'ERROR', 'ANULADO', 'INACTIVO', 'SALIDA', 'BAJA'].includes(e))
        return { label: cap(e), dot: 'bg-rose-500', text: 'text-rose-500', bg: 'bg-rose-50' };
    return { label: e ? cap(e) : '—', dot: 'bg-violet-500', text: 'text-violet-600', bg: 'bg-violet-50' };
}
    ...
    return (
        <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
                <p className="font-semibold text-slate-700 text-sm truncate max-w-[200px]">{name || '—'}</p>
                {sub != null && sub !== '' && <p className="text-[11px] text-slate-400 font-mono truncate max-w-[200px]">{sub}</p>}
            </div>
        </div>
    );
}
